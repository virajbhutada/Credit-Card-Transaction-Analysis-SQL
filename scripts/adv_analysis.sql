...
### 5. Customer Retention & Churn Prediction:
   ...
   - Churn Prediction Model: If you have data on when customers cancel or stop using their cards, you could develop a churn prediction model using features like spending 
                             trends, repayment history, overdue balances, and the time since the last transaction. These features help identify customers at risk of churning, enabling proactive 
                             retention efforts.
    ...
    r.total_repayment_amount,
    r.overdue_balance,
    CASE 
        WHEN MAX(t.transaction_date) < CURDATE() - INTERVAL 6 MONTH THEN 1 -- Likely churned
        ELSE 0 -- Active customer
    END AS churn_flag
    FROM 
    customers c
    LEFT JOIN 
